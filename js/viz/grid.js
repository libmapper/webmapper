...
function GridView(container, model)
{
    ...
    // create empty preset
    this.viewPresets = [new GridViewPreset('Default Preset', [], [])];
...
*/
        ...
        $("#devGrid").on("map", function(e, cell){
            e.stopPropagation();    //prevents bubbling to main.js
            _self.link(e, cell);
        });
        $("#devGrid").on("unmap", function(e, cell){
            e.stopPropagation();    //prevents bubbling to main.js
            _self.unlink(e, cell);
        });
        $("#devGrid").on("toggleMap", function(e, cell){
            e.stopPropagation();    //prevents bubbling to main.js
        ...
        $("#sigGrid").on("map", function(e, cell){
            e.stopPropagation();    //prevents bubbling to main.js
            _self.map(e, cell);
        });
        $("#sigGrid").on("unmap", function(e, cell){
            e.stopPropagation();    //prevents bubbling to main.js
            _self.unmap(e, cell);
        });
        $("#sigGrid").on("toggleMap", function(e, cell){
            e.stopPropagation();    //prevents bubbling to main.js
            _self.toggleMap(e, cell);
        });
        ...
        $("#devGrid").on("updateMapProperties", function(e){
            e.stopPropagation();    //prevents bubbling to main.js
            // only sig grid needs this function
        });
        $("#sigGrid").on("updateMapProperties", function(e){
            e.stopPropagation();    //prevents bubbling to main.js
            // trigger update topMenu event
            $(_self._container).trigger("updateMapProperties");
        });
    ...
    map : function (e, cell) {
        ...
        if (this.model.isMapped(src, dst) == false) {
            // trigger map event
            $(this._container).trigger("map", [src, dst]);
        }
    },

    unmap : function (e, cell) {
        var src = cell.getAttribute("data-src");
        var dst = cell.getAttribute("data-dst");
    ...
    link : function (e, cell) {
        ...
        if (this.model.isLinked(src, dst) == false) {
            // trigger link event
            $(this._container).trigger("link", [src, dst]);
        }
    },
    ...
    unlink : function (e, cell) {
        ...
        if (this.model.isLinked(src, dst) == true) {
            ...
            $(this._container).trigger("unlink", [src, dst]);
        }
    },
    ...
    toggleLink : function (e, cell) {
        ...
        // toggle the link
        if (this.model.isLinked(src, dst)) {
            // already linked, so unlink
            $(this._container).trigger("unlink", [src, dst]);
        }
        else {
            // trigger link event
        ...
        // 'ALT + left/right to move the active grid
            ...
            if (this.viewMode == 0) {
                if (e.keyCode == 37) {
                    ...
                    // right
                    // only if there are signals
                    if (   this.includedSrcs.length > 0
                        && this.includedDsts.length > 0)
                        this.setActiveGrid(1);
                }
            }
        }
    ...
    updateSignalsGrid : function() {
        ...
        var c = this.model.maps.keys();
        for (var i = 0; i < c.length; i++) {
            var map = this.model.maps.get(c[i]);
            var src = map.src;
            var dst = map.dst;
            maps.push([src, dst]);
        }

        this.sigGrid.updateDisplayData(srcSigs, dstSigs, maps);
    ...
    calculateSizes : function () {
        ...
        var h = ($(this._container).height() - $("#actionBar").height()
                 - $("#logoWrapper").height() - 2);
    ...
    saveViewPreset : function() {
        ...
        var newPreset = new GridViewPreset(name, this.includedSrcs.slice(0),
                                           this.includedDsts.slice(0));
