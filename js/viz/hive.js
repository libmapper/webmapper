...
function HivePlotView(container, model)
{
    ...
    this.groupColors = ["Cyan", "Orange", "Yellow", "Red", "DodgerBlue",
                        "PeachPuff", "BlanchedAlmond", "DarkViolet",
                        "PaleGreen", "Silver", "AntiqueWhite", "LightSteelBlue"];
    this.pColors;
            ...
            for (var index = 0; index < lis.length; index ++) {
                ...
                if (!doNotRecurse || node == lis[index].parentNode) {
                    ...
                    if (lis[index].addEventListener) {
                        lis[index].addEventListener('mousedown',
                                                    function (e) { e.preventDefault(); }, false);
                    }
                    else {
                        lis[index].attachEvent('onselectstart',
                ...
                if (lis[index].addEventListener) {
                    lis[index].addEventListener('click',
                                                createClickListener(lis[index]), false);
                }
                else {
...
//        div.setAttribute("style", "width: "+ (this.svgDim[0] ) + "px; height: "+ (this.actionBarHeight - (2*this.actionBarPadding)) + "px; padding: " + this.actionBarPadding + "px; ");
    ...
    draw : function() {
        ...
        // adapted hive plot
            ...
            this.drawAxes2(this.devs[0], 0, origin[0], origin[1], innerDim[0],
                           innerDim[1], outerDim[0], outerDim[1], 195, 345);
            this.drawAxes2(this.devs[1], 1, origin[0], origin[1], innerDim[0],
                           innerDim[1], outerDim[0], outerDim[1], 15, 165);
        }
        // not used anymore (adapted hive plot with a different center)
            ...
            this.drawAxes2(this.devs[0], 0, origin[0], origin[1], innerDim[0],
                           innerDim[1], outerDim[0], outerDim[1], 285, 345);
            this.drawAxes2(this.devs[1], 1, origin[0], origin[1], innerDim[0],
                           innerDim[1], outerDim[0], outerDim[1], 15, 85);
        }
    ...
    drawInclusionTable : function () {
        ...
        for (var ind = 0; ind < 2; ind++) {
            ...
            for (var i = 0; i < this.devs[ind].length; i++) {
                ...
                /* If not matched, below we check for signals that match and
                 * include the device only if there's at least 1 matched signal */
        ...
        for (var ind = 0; ind < 2; ind++) {
            if (this.selectedCells[ind].length > 0) {
                ...
                label.setAttribute("class",
                                   (ind == 0)? "hive_srcLabel" : "hive_dstLabel");
...
//        var nodePadding = (l - (nodeGroupPadding * (nTotalDevs - 1)) - (2 * axisPadding)) / nTotalSigs;
        ...
        for (var i = 0; i < sigsArray.length; i++) {
            ...
            for (var j = 0; j < signals.length; j++) {
                ...
                node.setAttribute("data-src", sig.device);
                ...
                if (arrIsUnique(sig.device, this.excludedDevs[ind]))
    ...
    drawAxes2 : function (srcData, ind, originX, originY, w1, h1,
                          w2, h2, angle1, angle2) {
        var _self = this;
    ...
    drawMaps : function() {
        ...
        for (var i = 0; i < this.sigs[0].length; i++) {
            for (var j = 0; j < this.sigs[1].length; j++) {
                ...
                var src = s.device + '/' + s.name;
                var dst = d.device + '/' + d.name;
                if (this.model.isMapped(src, dst)) {
                    ...
                    var line = document.createElementNS(this.svgNS, "path");
                    var key = s.device + s.name + ">" + d.device + d.name;
                    line.setAttribute("data-fullname", key);
                    line.setAttribute("data-src", s.device);
                    line.setAttribute("data-dst", d.device);
...
//                    line.setAttribute("d", "M " + x1 + " " + y1 + " L " + x2 + " " + y2);
                    ...
                    if (   arrIsUnique(s.device, this.excludedDevs[0])
                        && arrIsUnique(d.device, this.excludedDevs[1]))
                    ...
                    if (this.selectedMaps_getIndex(key) != -1) {
                        ...
                        this.model.selectedMaps_toggleMap(key, null);
                    }
    ...
    onDevMouseOver : function(devName) {
        ...
        for (var i = 0; i < this.mapLines.length; i++) {
            ...
            if (  con.getAttribute("data-src") == devName
               || con.getAttribute("data-dst") == devName) {
        ...
        for (var i = 0; i < this.mapLines.length; i++) {
            ...
            if (con.getAttribute("data-src") == devName || con.getAttribute("data-dst") == devName ) {
                if (   arrIsUnique(con.getAttribute("data-src"), this.excludedDevs[0])
                    && arrIsUnique(con.getAttribute("data-dst"), this.excludedDevs[1]) )
    ...
    onNodeMouseOver : function(node) {
        ...
        for (var i = 0; i < this.nodes[ind].length; i++) {
            ...
            if (   node.getAttribute("data-src") == node2.getAttribute("data-src")
                && node.getAttribute("data-srcSignal") == node2.getAttribute("data-srcSignal"))
    ...
    compareSignalLabel : function (devA, devB) {
        var a = devA.device.toUpperCase() + devA.name.toUpperCase();
        var b = devB.device.toUpperCase() + devB.name.toUpperCase();
        return a.localeCompare(b);
    },
    ...
    onNodeClick : function(node) {
        // nodes and maps can't be selected at the same time
        ...
        if (false) {
            // disabling selecting multiple nodes for now
            if (this.selectedCells_getCellIndex(node, ind) == -1) {
        ...
        // if COMMAND is not pressed, then user is selecting a single cell
            ...
            if (this.selectedCells[ind].length == 1){
                ...
                // case: eith zero or many, so clear all and set the current
                this.selectedCells_clearAll(ind);
                this.selectedCells_addCell(node, ind);
            }
        }
        ...
        if (   this.selectedCells[0].length == 1
            && this.selectedCells[1].length == 1) {
            var src = (  this.selectedCells[0][0].getAttribute("data-src")
                       + this.selectedCells[0][0].getAttribute("data-srcSignal"));
            var dst = (  this.selectedCells[1][0].getAttribute("data-src")
                       + this.selectedCells[1][0].getAttribute("data-srcSignal"));
            if (this.model.isMapped(src, dst)) {
    ...
    selectedCells_getCellIndex : function (cell, ind) {
        var index = -1;
        var fullname = (  cell.getAttribute("data-src")
                        + cell.getAttribute("data-srcSignal"));
        for (var i = 0; i < this.selectedCells[ind].length; i++) {
            var cell2 = this.selectedCells[ind][i];
            var name = (  cell2.getAttribute("data-src")
                        + cell2.getAttribute("data-srcSignal"));
            if (fullname == name) {
                index = i;
                break;
            }
    ...
    onMapClick : function(con) {
        ...
        if (arrIsUnique(name, this.selectedMaps)) {
            ...
            fakeSrc.setAttribute("data-srcSignal",
                                 con.getAttribute("data-srcSignal"));
            ...
            fakeDst.setAttribute("data-srcSignal",
                                 con.getAttribute("data-dstSignal"));
