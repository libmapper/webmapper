...
GridDevices.prototype.refresh = function () {
    ...
    for (var index=0; index< this.filteredData[0].length; index++)
    {
        ...
        // I don't know why -1 .... getBBox() doesn't really work well
        var xPos = ((this.nCols) * (this.cellDim[0] + this.cellMargin)
                    + Math.floor(this.cellDim[0] / 2) - 1 );
    ...
    for (var index=0; index< this.filteredData[1].length; index++) {
        ...
        label.setAttribute("y", (this.nRows) * (this.cellDim[1] + this.cellMargin)
                           + Math.ceil(this.cellDim[1] / 2) + 2 );
    ...
    for (var i = 0; i < this.nRows; i++) {
        for (var j = 0; j < this.nCols; j++) {
            ...
            for (var k = 0; k < this.selectedCells.length; k++) {
                ...
                if (   c.getAttribute("data-src") == src
                    && c.getAttribute("data-dst") == dst) {
...
GridDevices.prototype.onLabelClick = function (evt, _self)
{
    ...
    // if COMMAND is not pressed, then user is selecting a single cell
        ...
        if (_self.selectedLabels.length == 0) {
            // case: no selected cell
            _self.selectedLabels_addLabel(label);
        }
        else if (_self.selectedLabels.length == 1) {
            // case: one selected cell
            if (_self.selectedLabels_getLabelIndex(label) == -1) {
