...
function GridSVG()
{
    ...
    this.handleClicked;             // helpers for zooming scroll bars
    this.handleClick;
    this.handleValues;
    ...
    this.selectedCells = [];        // reference to the selected cells
    this.mousedOverCell = null;     // reference to the cell that's moused over
...
GridSVG.prototype = {
    preInit : function(container, model, gridIndex) {
        ...
        // vbox width-height dimensions
        this.vboxDim = [ this.svgDim[0], this.svgDim[1] ];
    ...
    initDimensions : function(container) {
        ...
        this.svgDim[0] = (w - this.rowLabelsW - this.scrollBarDim[0]
                          - this.labelMargin);
        // dimensions of the svg canvas
        this.svgDim[1] = (h - this.colLabelsH - this.scrollBarDim[1]
                          - this.labelMargin);
        ...
        // aspect ratio of SVG viewbox (for zooming)
        this.aspect = this.svgDim[0] / this.svgDim[1];

        // aspect ratio of col viewbox (for zooming)
        this.aspectCol = this.svgDim[0] / this.colLabelsH;

        // aspect ratio of row viewbox (for zooming)
        this.aspectRow = this.rowLabelsW / this.svgDim[1];
    },
    init : function() {
        $(this._container).empty();

        var div;
        // to pass to the instance of LibMApperMatrixView to event handlers
        var _self = this;
        ...
        this.svgColLabels.setAttribute("style", "float: left; clear:both;"
                                       +"margin-left:"
                                       + (this.rowLabelsW + 42).toString() + "px;"
                                       +"margin-bottom: " + this.labelMargin +"px");
...
// wrapper TOP
        ...
        div.setAttribute("style", "width: 14px; height: 14px;"
                         +"position: relative; float: left;"
                         +"margin: 2px 12px 2px "
                         + (this.rowLabelsW+17).toString() + "px");
        div.setAttribute("class", "inactiveGridIndicator");
        div.title = "a black square indicates grid is active and will receive"
                    +" keyboard shortcuts (ALT + left/right arrow to switch)";
        wrapper0.appendChild(div);

        //horizontal scrollbar
        div = document.createElement("div");
        div.setAttribute("id", "hZoomSlider" + this.gridIndex);
        div.setAttribute("style", "position: relative; clear:right;"
                         +"margin-left: 2px; float: left; margin-bottom: 10px;");
        wrapper0.appendChild(div);
...
// END wrapper TOP
        ...
        this.svgRowLabels.setAttribute("style", "float:left; margin-right: "
                                       + this.labelMargin +"px");
        ...
        this.svg.setAttribute("viewBox",
                              this.toViewBoxString(this.vboxPos[0],
                                                   this.vboxPos[1],
                                                   this.vboxDim[0],
                                                   this.vboxDim[1]));
        ...
        btn = document.createElement("button");
        btn.innerHTML = "&#9632;";
        btn.setAttribute("style", "font-size: 22px; padding: 0px 6px 1px;"
        ...
        var filterTooltips = ["filter columns by regular expression",
        ...
        for (var ind = 0; ind < 2; ind++) {
            ...
            filter.addEventListener("keyup", function(evt){
                ...
                // send to parent Grid.js to store
                $(this._container).trigger("filterChanged", _self.filters);
                _self.filterData();
                _self.refresh();
            });
...
// END button bar
        ...
        for (var i = 0; i < this.cells.length; i++) {
            ...
            if (className.indexOf("cell_connected") == -1) {
                ...
                curCell.setAttribute("class", className
                                     + curCell.getAttribute("defaultClass"));
            }
        }
     ...
     * handle removing the previously selected cell and the special case
     * where the previous was null */
    onCellClick : function(evt, _self) {
        ...
        if (evt.metaKey) {
            /* COMMAND key on MAC, CONTROL key on PC is pressed, user is
             * adding/removing to selection. */
            if (_self.selectedCells_getCellIndex(cell) == -1) {
            ...
            // COMMAND not pressed: user is selecting a single cell
            if (_self.selectedCells.length == 0) {
                // case: no selected cell
                _self.selectedCells_addCell(cell);
            }
            else if (_self.selectedCells.length == 1) {
                // case: one selected cell
                if (_self.selectedCells_getCellIndex(cell) == -1) {
                ...
                // case: more than one selected cell
                _self.selectedCells_clearAll();
                _self.selectedCells_addCell(cell);
            }
        }
    ...
    selectedCells_getCellIndex : function(cell) {
        ...
        return index;
        // not sure but might be dangerous
        //return this.selectedCells.indexOf(cell);
    },
    ...
    getCellByPos : function(row, col) {
        for (var i = 0; i < this.cells.length; i++){
            if (   this.cells[i].getAttribute('data-row') == row
                && this.cells[i].getAttribute('data-col') == col) {
                return this.cells[i];
            }
        }
    ...
    /**
     * helper function that customizes JQueryUI Range slider to gain
     * scrollbar functionality when slider is clicked, check if clicked one
     * of the handles or the slider if slider, store the original values
     * (need them saved otherwise slider's default functionality overrides
     * them) and the click position (used to determine how much user has
     * dragged the mouse)
     */
    sliderClicked: function(e, ui, _self) {
        // true if clicked on handle, false for slider
        _self.handleClicked = $(e.originalEvent.target).hasClass("ui-slider-handle");

        // store the original values
        _self.handleValues = ui.values;

        // store the initial x/y click position
        _self.handleClick = [e.pageX, e.pageY];
    },

    /**
     * function that takes care of scrolling and zooming functionality for
     * both zoom bars param ind: 0 is X dimension, 1 is Y dimension.
    ...
    zoomSlide : function(_self, sliderID, ui, ind, mousePos) {
        ...
        if (_self.handleClicked) {
            ...
            if (   w < _self.vboxMinDim[ind]
                || w > _self.vboxMaxDim[ind]
                || w > _self.contentDim[ind]) {
                // do nothing if range is beyond min/max or larger than contentDim
                return false;
            }
                ...
                if (_self.vboxDim[ind] - this.zoomIncrement > _self.contentDim[ind]) {
                    ...
                    // other dimension's size based on aspect ratio
                    _self.vboxDim[1-ind] = (ind == 0) ? w/_self.aspect: w*_self.aspect;
                    // set the vbox position based on value of first handle (reversed for Y)
                    _self.vboxPos[ind] = ((ind == 0) ? ui.values[0]
                                          : _self.contentDim[ind] - ui.values[1]);

                    /* if other dimension gets out of range, must adjust the
                     * vbox's position appropriately */
                    if (_self.vboxPos[1-ind]
                        + _self.vboxDim[1-ind] > _self.contentDim[1-ind]) {
                        // when other dimension is at max, keep the view box at 0
                        if (_self.vboxPos[1-ind] <= 0)
                            _self.vboxPos[1-ind] = 0;
                        else {
                            // when other dimension is not maxed, scroll left by the surpassed amount
                            var overflow = (_self.contentDim[1-ind]
                                            - _self.vboxPos[1-ind]
                                            - _self.vboxDim[1-ind]);
...
//                        _self.updateZoomBars();
        ...
        // for when the range was clicked (scroll and slide)
            ...
            // reversed for Y
            var mouseDiff = ((ind == 0) ? mousePos - _self.handleClick[ind]
                             : _self.handleClick[ind] - mousePos);

            // drag size relative to size of the scroll bar, multiplied by scrollable range
            var dx = ((mouseDiff / _self.svgDim[ind])
                      * (_self.contentDim[ind]-_self.vboxDim[ind]));
            var v1 = _self.handleValues[0]+dx;    // 1st handle new value
            var v2 = _self.handleValues[1]+dx;    // 2nd handle new value

            /* fixes glitch when user scrolls with mouse fast beyond min/max
             * by removing the excess and snapping to the min/max. */
            var overflow=0;
            if (v1 < 0)
                overflow = 0 - v1;
            ...
            _self.vboxPos[ind] = ((ind==0) ? v1
                                  : (_self.contentDim[ind] - _self.vboxDim[ind]) - v1);
            _self.updateViewBoxes();
        }
    },
    ...
    zoomIn : function() {
        ...
        if (this.vboxDim[0] - this.zoomIncrement > this.vboxMinDim[0]) {
            ...
            // bad to redraw everything but needed for now to change font size
            this.refresh();
        }
        else {
...
//                this.updateZoomBars();
    ...
    zoomOut : function() {
        ...
        if (this.vboxDim[0] + this.zoomIncrement < this.vboxMaxDim[0]
            && this.vboxDim[1] + (this.zoomIncrement / this.aspect) < this.vboxMaxDim[1]) {
            var ratio = this.zoomIncrement/this.vboxDim[0];
            this.vboxDim[0] += this.zoomIncrement;
            this.vboxDim[1] += this.zoomIncrement/this.aspect;
            this.fontSize = this.fontSize + (this.fontSize*ratio);
    ...
    keyboardHandler: function(e) {
        ...
        // movement arrows to move the selected cell
            ...
            if (this.selectedCells.length == 0) {
                ...
                // trigger mouseover events for the new selected cell
                this.triggerMouseOver(newCell);
                this.vboxPos = [0, 0];
            }
            else {
                // there is a previously selected cell(s)
                var m = 1;    // cell jump size

                // if shift key is pressed, increase the jump size;
                if (e.shiftKey === true)
                    m = 3;

                // get position of the currently selected cell
                var lastSelected = this.selectedCells[this.selectedCells.length-1];
                var currentPos = [parseInt(lastSelected.getAttribute('data-row')),
                                  parseInt(lastSelected.getAttribute('data-col'))];

                // clear all selected cells when arrows are pressed
                this.selectedCells_clearAll();

                // set position of the new selected cell
                // [row, col]... I know very confusing with X/Y coordinates
                ...
                // trigger mouse out event for previous selected cells
                this.onCellMouseOut();
                ...
                /* helper for code re-useability, set in switch statement
                 * following (0=left/right=x, 1=up/down=y) */
                var dim;
                    ...
                    case 40:    // down
                        ...
                        if (this.vboxDim[dim] < this.contentDim[dim]) {
                            ...
                            if (pos[dim] < this.vboxPos[dim] + ((m - 1) * cellW)) {
                                ...
                                this.vboxPos[dim] = pos[dim] - ((m - 1) * cellW);
                            }
                            if (this.vboxPos[dim] < 0) {
                                // if moved to less than 0, set to 0
                                this.vboxPos[dim] = 0;
                            }
                            else if (pos[dim] > this.vboxPos[dim]
                                     + this.vboxDim[dim] - (m * cellW)) {
                                // off screen on right/down
                                // set the new position
                                this.vboxPos[dim] = pos[dim] - this.vboxDim[dim] + (m * cellW);
                            }
                            if (this.vboxPos[dim] > this.contentDim[dim] - this.vboxDim[dim]) {
                                // if moved outside of content, set to the max
    ...
    updateViewBoxes : function() {
        this.svg.setAttribute("viewBox",
            this.toViewBoxString(this.vboxPos[0],
                                 ...
                                 this.vboxDim[0],
                                 this.vboxDim[1]));
        this.svgColLabels.setAttribute("viewBox",
            this.toViewBoxString(this.vboxPos[0],
                                 this.colLabelsH - (this.vboxDim[0] / this.aspectCol),
                                 this.vboxDim[0],
                                 this.vboxDim[0] / this.aspectCol));
        this.svgRowLabels.setAttribute("viewBox",
            this.toViewBoxString(this.rowLabelsW - (this.vboxDim[1] * this.aspectRow),
                                 this.vboxPos[1],
                                 this.vboxDim[1] * this.aspectRow,
                                 this.vboxDim[1]));
    },
    ...
    updateZoomBars : function() {
        if (this.contentDim[0] == 0)
            ...
            $("#hZoomSlider" + this.gridIndex).slider("option", "values",
                [ this.vboxPos[0], this.vboxPos[0] + this.vboxDim[0]]);
        }
        ...
        if (this.contentDim[1] == 0)
            ...
            $("#vZoomSlider" + this.gridIndex).slider("option", "values",
                [this.contentDim[1] - (this.vboxPos[1] + this.vboxDim[1]),
                 this.contentDim[1] - this.vboxPos[1]]);
        }
    },
    ...
    refresh : function() {
        ...
        // style the cell
        /*
        if (this.mousedOverCell != null) {
            //style when mouse is over the toggled cell's row/col
