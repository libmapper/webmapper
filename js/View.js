                ...
                redraw = function(speed) {
                    ...
                    model.maps.each(function(map) {
                        if (!map.src.view || !map.dst.view) {
                            remove_object_svg(map, default_speed, 'linear');
                            return;
                        }
                        let src = map.src.position;
                        let dst = map.dst.position;
...
//                        return;
                ...
                redraw = function(speed, update_tables) {
                    ...
                    model.maps.each(function(map) {
                        ...
                        if (srctab == dsttab) {
                            if (src.top == dst.top) {
                                // same row
                                let offset = src.left > mp ? -src.height : src.height;
                    ...
                    if (first_transition) {
                        ...
                        fileRep.view = svgArea.circle(svg_frame.cx, svg_frame.cy, 0)
                               .attr({'fill': 'black', 'stroke': 'white'})
                               .animate({'r': 100}, default_speed, 'linear');
                        fileRep.view.hover(
                            function() {
                        ...
                        fileRep.view.click(function() {
                            ...
                            fileRep.label.animate({'fill-opacity': 0}, default_speed,
                                                   'linear', function() {
                                this.remove();
                                fileRep.label = null;
                            });
                            fileRep.view.animate({'r': 0}, default_speed, 'linear',
                                                 function() {
                                this.remove();
                                fileRep.view = null;
                            });
    ...
    function new_pos(x, y) {
        return { 'x': x != null ? x : Math.random() * container_frame.width,
                 'y': y != null ? y : Math.random() * container_frame.height };
    }
    ...
    this.filter_signals = function(searchbar, text) {
        ...
        if (searchbar == 'srcSearch') {
            ...
            if (currentView == 'list' || currentView == 'canvas')
                leftTable.filter_text(text);
            else if (currentView == 'grid')
                topTable.filter_text(text);
        }
        else {
            dstregexp = text ? new RegExp(text, 'i') : null;
            if (currentView == 'list')
                rightTable.filter_text(text);
            else if (currentView == 'grid')
                leftTable.filter_text(text);
        }
        redraw(default_speed, true);
